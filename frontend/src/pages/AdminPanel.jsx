...
import { Loader2, ShieldCheck, Users, Landmark, AlertCircle, Trash2, Copy, Download, Heart, Calendar } from 'lucide-react';
...
const AdminPanel = () => {
  ...
  const [donationSummary, setDonationSummary] = useState({ totalRaised: 0, totalDonors: 0, monthlyRaised: 0 });
  const [donationPage, setDonationPage] = useState(1);
  const [donationPages, setDonationPages] = useState(1);
  ...
  const fetchAdminData = async () => {
    try {
      ...
      // Calculate counts
      setStats({
        users: rawDonors.length + rawBanks.length + 5,
  ...
  const fetchDonations = async (page = 1) => {
    ...
    try {
      const { data } = await api.get(`/financial-donations/admin/all?page=${page}`);
      if (data.success) {
        setDonations(data.donations);
        setDonationPages(data.pages);
        setDonationSummary(data.summary);
        setDonationPage(data.page);
      }
    } catch (err) {
      toast.error('Failed to retrieve donation records');
    } finally {
  ...
  useEffect(() => {
    if (activeTab === 'donations') {
      fetchDonations(donationPage);
    }
  }, [activeTab, donationPage]);
  ...
  const handleExportCSV = async () => {
    try {
      const response = await api.get('/financial-donations/admin/export', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'financial_donations.csv');
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10 w-full text-left font-sans">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/5 pb-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Platform moderation, verification center, and financial audit logs</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex border-b border-slate-200 dark:border-white/5">
          <button
            ...
            className={`px-6 py-3 font-semibold text-sm transition border-b-2 cursor-pointer ${
              activeTab === 'verification'
                ? 'border-ob-red-500 text-ob-red-500'
          ...
          <button
            ...
            className={`px-6 py-3 font-semibold text-sm transition border-b-2 cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'donations'
                ? 'border-ob-red-500 text-ob-red-500'
                : 'border-transparent text-slate-400 hover:text-slate-300'
            }`}
        ...
        {activeTab === 'verification' ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 p-5 rounded-2xl flex items-center justify-between shadow-sm">
                <div>
                  <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold uppercase block">Total Raised</span>
                  <span className="text-2xl font-black font-mono text-ob-red-500 mt-1 block">₹{donationSummary.totalRaised.toLocaleString('en-IN')}</span>
                </div>
                <div className="p-3.5 bg-red-500/10 text-ob-red-500 rounded-2xl"><Landmark className="w-6 h-6" /></div>
              </div>

              <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 p-5 rounded-2xl flex items-center justify-between shadow-sm">
                <div>
                  <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold uppercase block">Total Donors</span>
                  <span className="text-2xl font-black font-mono text-blue-500 mt-1 block">{donationSummary.totalDonors}</span>
                </div>
                <div className="p-3.5 bg-blue-500/10 text-blue-500 rounded-2xl"><Users className="w-6 h-6" /></div>
              </div>

              <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 p-5 rounded-2xl flex items-center justify-between shadow-sm">
                <div>
                  <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold uppercase block">This Month</span>
                  <span className="text-2xl font-black font-mono text-emerald-500 mt-1 block">₹{donationSummary.monthlyRaised.toLocaleString('en-IN')}</span>
                </div>
                <div className="p-3.5 bg-emerald-500/10 text-emerald-500 rounded-2xl"><Calendar className="w-6 h-6" /></div>
              </div>
            </div>

            {/* Table and Actions Container */}
            <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 p-6 rounded-2xl shadow-sm dark:shadow-xl space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 dark:border-white/5 pb-4">
                <div>
                  <h2 className="text-base font-bold text-slate-900 dark:text-white">Transaction Logs</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400">View and audit all financial donations made to OneBlood.</p>
                </div>
                
                <button
                  onClick={handleExportCSV}
                  disabled={donations.length === 0}
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-2 transition cursor-pointer"
                >
              ...
              <div className="overflow-x-auto">
                {donationLoading ? (
                  <div className="py-12 flex flex-col items-center justify-center">
                    ...
                    <p className="text-xs text-neutral-400">Retrieving transactions...</p>
                  </div>
                ) : donations.length === 0 ? (
                  <p className="text-xs text-slate-500 italic text-center py-12">No donations recorded yet.</p>
                ) : (
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="text-slate-500 border-b border-slate-200 dark:border-white/5 pb-3">
                        <th className="py-3 font-semibold uppercase tracking-wider">Receipt ID</th>
                        <th className="py-3 font-semibold uppercase tracking-wider">Donor Name</th>
                        <th className="py-3 font-semibold uppercase tracking-wider">Email</th>
                    ...
                    <tbody>
                      {donations.map((d) => (
                        <tr key={d._id} className="border-b border-slate-200 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                          <td className="py-3.5 font-mono text-ob-red-500 font-bold">{d.receiptId}</td>
                          <td className="py-3.5 font-semibold text-slate-900 dark:text-white">{d.donorName}</td>
                          <td className="py-3.5 text-slate-500">{d.donorEmail}</td>
                          <td className="py-3.5 font-bold text-slate-900 dark:text-white">₹{d.amountInRupees.toLocaleString('en-IN')}</td>
                          <td className="py-3.5">
                          ...
                          <td className="py-3.5 text-slate-500">{new Date(d.createdAt).toLocaleDateString('en-IN')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* Pagination Controls */}
              {donationPages > 1 && (
                <div className="flex justify-center items-center gap-4 pt-4 border-t border-slate-200 dark:border-white/5">
                  <button
                    disabled={donationPage === 1 || donationLoading}
                    onClick={() => setDonationPage(donationPage - 1)}
                    className="px-3.5 py-1.5 border border-ob-glass-border rounded-lg text-xs hover:bg-white/5 disabled:opacity-30 cursor-pointer"
                  >
                    Previous
                  </button>
                  <span className="text-xs text-neutral-400">Page {donationPage} of {donationPages}</span>
                  <button
                    disabled={donationPage === donationPages || donationLoading}
                    onClick={() => setDonationPage(donationPage + 1)}
                    className="px-3.5 py-1.5 border border-ob-glass-border rounded-lg text-xs hover:bg-white/5 disabled:opacity-30 cursor-pointer"
                  >
                    Next
