...
import { Loader2, ShieldCheck, Users, Landmark, AlertCircle, Trash2, CheckCircle, XCircle } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center bg-slate-950 text-white">
        ...
        <p className="text-xs text-slate-400">Loading admin panel logs...</p>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10 w-full text-left font-sans">
        ...
        <div className="flex items-center space-x-4 border-b border-white/5 pb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Platform moderation, verification center, and request logs</p>
          </div>
        </div>

        {/* Stats Row */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="bg-slate-900/60 border border-white/5 p-4 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              ...
              <span className="text-xl font-bold font-mono">{stats.donors}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-white/5 p-4 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              ...
              <span className="text-xl font-bold font-mono">{stats.banks}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-white/5 p-4 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              ...
              <span className="text-xl font-bold font-mono">{stats.requests}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-white/5 p-4 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              ...
              <span className="text-xl font-bold font-mono">{banks.filter(b => b.isVerified).length}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6 bg-slate-900/60 border border-white/5 p-6 rounded-2xl shadow-xl space-y-4">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-400 border-b border-white/5 pb-2">Blood Bank Verifications</h2>
            ...
            <div className="space-y-3 overflow-y-auto max-h-[400px]">
              {banks.length === 0 ? (
                ...
                banks.map(bank => (
                  <div key={bank._id} className="p-4 bg-white/5 border border-white/5 rounded-xl flex justify-between items-center gap-4">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-white truncate">{bank.name}</p>
                      <p className="text-[10px] text-slate-500 truncate">{bank.registrationNumber} | {bank.city}</p>
                    </div>
                    <button
                      onClick={() => handleVerifyBank(bank._id, bank.isVerified)}
                      disabled={actionLoading === bank._id}
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer ${bank.isVerified ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30' : 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30'}`}
                    >
          ...
          <div className="lg:col-span-6 bg-slate-900/60 border border-white/5 p-6 rounded-2xl shadow-xl space-y-4">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-400 border-b border-white/5 pb-2">Active Emergency Requests</h2>
            
            <div className="space-y-3 overflow-y-auto max-h-[400px]">
              {requests.length === 0 ? (
                <p className="text-xs text-slate-500 italic text-center py-4">No active emergency requests found.</p>
              ) : (
                requests.map(req => (
                  <div key={req._id} className="p-4 bg-white/5 border border-white/5 rounded-xl flex justify-between items-center gap-4">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-white truncate">{req.patientName}</p>
                      <p className="text-[10px] text-slate-500 truncate">
                        Need <span className="text-red-500 font-bold">{req.bloodGroup}</span> at {req.hospitalName}
                      </p>
                    </div>
                    <button
                      onClick={() => handleDeleteRequest(req._id)}
                      disabled={actionLoading === req._id}
                      className="p-2 bg-red-600/10 hover:bg-red-600/20 text-red-500 rounded-lg transition-all cursor-pointer"
                    >
